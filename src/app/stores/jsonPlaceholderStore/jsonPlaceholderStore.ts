...
import { jsonPlaceholderApi } from '@/app/composables/jsonPlaceholderApi';
...
export const jsonPlaceholderStore = create<JsonPlaceholderInterface>()(
	(set, get) => ({
		posts: [],
		allPosts: [],
		loading: false,
			...
			const sortedPosts = posts.toSorted((prevPost, nextPost) => {
				if (prevPost && nextPost) {
					...
					if (typeof prevValue === 'string' && typeof nextValue === 'string') {
						return prevValue
							.toLowerCase()
							.localeCompare(nextValue.toLowerCase());
					} else if (
